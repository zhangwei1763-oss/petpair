...
function mapPetFromDB(row: any): PetProfile {
  return {
    ...
    personalityTags: Array.isArray(row.personality_tags) ? row.personality_tags : [],
    energyLevel: row.energy_level || 'medium',
    activityPreferences: Array.isArray(row.activity_preferences) ? row.activity_preferences : [],
    socialPreferences: Array.isArray(row.social_preferences) ? row.social_preferences : [],
    photos: Array.isArray(row.photos) ? row.photos : [],
    bio: row.bio || '',
    vaccineStatus: row.vaccine_status || 'none',
